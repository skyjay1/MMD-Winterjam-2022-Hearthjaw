...
public class MovingLightSourceGoal<T extends LivingEntity> extends Goal {
	...
	public void tick() {
		...
		for (int i = 0, n = Mth.ceil(entity.getBbHeight()) + 1; i <= n; i++) {
			...
			if (blockState.is(SWTRegistry.BlockReg.LIGHT.get()) && blockState.getValue(MovingLightBlock.LEVEL) >= lightLevel) {
				return;
			}
			...
			if (canPlaceLightAt(entity.level, blockState, pos)) {
				...
				BlockState light = SWTRegistry.BlockReg.LIGHT.get().defaultBlockState()
